* Program called by build60_08.do
...
keep if `varmatch'!=0
gen missing=1
save "$directmatch/aux.dta", replace
use "$directmatch/small$year0.dta", clear
sum `varmatch0' [w=dweght]
...
keep if `varmatch'!=0
gen missing=0
append using "$directmatch/aux.dta"
save "$directmatch/aux.dta", replace
...
drop rand `varmiss'
save "$directmatch/aux1.dta", replace
count

use "$directmatch/aux.dta", clear
keep if missing==0
drop idm*
...
sort numbering
save "$directmatch/aux0.dta", replace
count

use "$directmatch/aux1.dta", clear
merge m:1 numbering using "$directmatch/aux0.dta"
keep if _merge==3
drop _merge
...
sort id 
save "$directmatch/small$year1`varmiss'_supp.dta", replace
}

* do merge regardless of value of global impute
use "$directmatch/small$year1.dta", clear
sort id
merge 1:1 id using "$directmatch/small$year1`varmiss'_supp.dta"
drop _merge
